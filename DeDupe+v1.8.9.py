...
from PyQt6.QtWidgets import (
	...
	QCheckBox, QGroupBox, QComboBox, QTextEdit, QProgressBar, QMenu, QHeaderView, QTextBrowser
)
...
# Suppress annoying QBasicTimer warnings
...
Use the **Quick Include Presets** for fast setup.

---

**Show Video Resolution**
Use **Options → Show Video Resolution** to display video resolution (e.g. 3840×2160).

---

**Progress Animation & Theme**
Choose your preferred progress style and switch between Light/Dark theme in **Options**.
""")

		# Append custom HTML section with nice light blue link
		link_html = """
		<hr style="margin: 25px 0 15px 0;">
		<h3>Need Help?</h3>
		<p>Report issues or suggest features on GitHub: 
		<a href="https://github.com/ScriptedBits/DeDupePlus" 
		   style="color: #4da6ff; font-weight: bold; text-decoration: none;">
		github.com/ScriptedBits/DeDupePlus
		</a>
		</p>
		"""
		current_html = text.toHtml()
		text.setHtml(current_html + link_html)
...
class ResolutionLoaderThread(QThread):
	update_item = pyqtSignal(int, int, str)
	finished = pyqtSignal()
	...
	def run(self):
		if not self.show_resolution or not self.tree:
			self.finished.emit()
			return
			...
			for g in range(self.tree.topLevelItemCount()):
				...
				for c in range(group.childCount()):
					...
					if fullpath and self._is_video_file(child.text(1)):
						...
						if resolution and self._is_running:
							self.update_item.emit(g, c, resolution)
					
					QThread.msleep(8)  # Small delay to keep UI responsive

		except RuntimeError:
			# Tree was cleared while thread was running → safe to ignore
			pass
		except Exception as e:
			print(f"Resolution thread error: {e}")
		finally:
			self.finished.emit()
...
class ScanThread(QThread):
	...
	finished = pyqtSignal(object, int)	 # duplicates, total_files_scanned
	...
	def run(self):
		...
		if self.isInterruptionRequested():
			self.finished.emit(None, total_files)
		else:
			duplicates = {k: v for k, v in groups.items() if len(v) > 1}
			self.finished.emit(duplicates, total_files)
...
class DuplicateFinder(QMainWindow):
	def __init__(self):
		...
		refresh.clicked.connect(self.clear_results)
	...
	def _export_html(self, path):
		...
		# Add Resolution column header if enabled
		...
		for key, files in self.current_groups.items():
			if len(files) < 2:
				continue
			html += f'<tr class="group"><td colspan="4">{key}</td>'
			if self.show_resolution:
				html += "<td></td>"
			html += "</tr>"
			...
			for fullpath, size, basename in sorted(files, key=lambda x: x[1], reverse=True):
				...
				row = f"<tr><td></td><td>{basename}</td><td>{fullpath}</td><td>{mb:.2f}</td>"

				# Add resolution if option is enabled and available in tree
				if self.show_resolution:
					resolution = ""
					# Try to find the item in the tree to get displayed resolution
					for i in range(self.tree.topLevelItemCount()):
						group = self.tree.topLevelItem(i)
						for j in range(group.childCount()):
							child = group.child(j)
							if child.text(2) == fullpath:  # match by full path
								resolution = child.text(4)
								break
					row += f"<td>{resolution}</td>"
		...
		with open(path, "w", newline="", encoding="utf-8") as f:
			...
			for key, files in self.current_groups.items():
				...
				for fullpath, size, basename in files:
					mb = round(size / (1024 * 1024), 2)
					row = [key, basename, fullpath, f"{mb:.2f}"]

					if self.show_resolution:
						resolution = ""
						# Try to get resolution from tree
						for i in range(self.tree.topLevelItemCount()):
							group = self.tree.topLevelItem(i)
							for j in range(group.childCount()):
								child = group.child(j)
								if child.text(2) == fullpath:
									resolution = child.text(4)
									break
	...
	def on_scan_finished(self, duplicates: dict, total_files_scanned: int):
		...
		for key, files in duplicates.items():
			...
			for fullpath, size, basename in sorted(files, key=lambda x: x[1], reverse=True):
				...
				item.setToolTip(2, fullpath)   # Column 2 = Full Path
		...
		self.statusBar.showMessage(
			...
			f"Total duplicate files: {sum(len(v) for v in duplicates.values())} | "
	...
	def clear_results(self):
	   """Safely clear results and stop any running resolution thread"""
	   self.cleanup_resolution_thread()	  # Important: stop thread first
	   self.tree.clear()
	   self.current_groups = {}
	   self.scan_status_label.setText("Results cleared")
	...
	def cleanup_resolution_thread(self):
		...
		if hasattr(self, 'resolution_thread') and self.resolution_thread is not None:
			if self.resolution_thread.isRunning():
				...
				self.resolution_thread.wait(800)   # Wait up to 600ms
			self.resolution_thread = None

	def toggle_resolution(self):
			"""Toggle video resolution column with proper warning"""
			...
			if self.show_resolution:
				try:
					...
					QMessageBox.warning(self, "Missing Dependency",
						"Video resolution display requires the 'pymediainfo' package.\n\n"
						"Install it with:\n"
						"	pip install pymediainfo\n\n"
						"Then restart the application.")
					self.show_resolution = False
					self.resolution_action.setChecked(False)
					self.settings.setValue("show_resolution", False)
			...
			if self.current_groups:
				QMessageBox.information(self, "Refresh Required",
									  ...
									  "Please run the scan again to update the results.")
